...
code = code.replace(
  /<span className="text-transparent bg-clip-text bg-gradient-to-r from-\[\#C89B3C\] to-\[\#E6D2AD\]">Extraordinary\?<\/span>/,
  '<span className="text-orange-500">Extraordinary?</span>'
);

code = code.replace(
  /bg-orange-500\/80 backdrop-blur-md border border-orange-400\/50 text-gray-900/,
...
// Top Section Location - remove bg-[#d1f2e5] so it merges with the parent or page? Wait, parent is App wrapper. 
// App wrapper is bg-[#fae6d8] (peach).
// But they said "BOTH OF THEM MAKE A MINT GREEN COLOUR". So the top section `div:nth-of-type(2)` and hero banner `div:nth-of-type(3)` are both mint green, and bottom (Search Bar) is same.
// Let's ensure `div:nth-of-type(3)` is `bg-transparent` since the inner div is now `bg-[#b5e8d5]`.
code = code.replace(
