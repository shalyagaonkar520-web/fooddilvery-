...
// Replace Leaflet imports and MapContainer with a simple modal
const simplePicker = `import React, { useState, useEffect } from 'react';
...
export default function LocationPicker() {
  const { isLocationPickerOpen, closeLocationPicker, updateLocation, deliveryLocation } = useLocationStore();
  ...
  const selectSearchResult = (result: any) => {
    ...
    // Calculate distance
    const dist = selectedCity ? haversineDistance(selectedCity.lat, selectedCity.lng, lat, lon) : 0;
    
    updateLocation(lat, lon, result.display_name, parseFloat(dist.toFixed(1)));
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          const dist = selectedCity ? haversineDistance(selectedCity.lat, selectedCity.lng, latitude, longitude) : 0;
          updateLocation(latitude, longitude, address, parseFloat(dist.toFixed(1)));
...
fs.writeFileSync('src/components/LocationPicker.tsx', simplePicker, 'utf8');
